...
## CI/CD to EKS with Helm + ArgoCD (GitHub Actions OIDC) — Blue/Green

Workflow: [.github/workflows/eks-helm-argocd-oidc.yml](.github/workflows/eks-helm-argocd-oidc.yml)
Flow: builds/pushes image → deploys with Helm → ArgoCD sync → AWS auth via OIDC. Blue/green is driven by Helm values and `activeColor`.

### Helm assets and blue/green
- Chart: [`chart/`](chart/Chart.yaml) with [`values.yaml`](chart/values.yaml), [Deployment](chart/templates/deployment.yaml), [Service](chart/templates/service.yaml), helpers in [`_helpers.tpl`](chart/templates/_helpers.tpl).
- Deployments: two, labeled `component: blue` and `component: green` (both can exist). Tags per color come from `colors.blue.tag` and `colors.green.tag` (fallback to `image.tag`).
- Service: selects `component: <activeColor>`; switch traffic by setting `activeColor` to `blue` or `green`.
- Default: app on `8080` ([`server.js`](server.js:6)), Service on `80` → targetPort `8080`.
...
- Helm: `EKS_NAMESPACE`, `HELM_RELEASE`, `HELM_CHART_PATH` (use `./chart`), `IMAGE_REPO` (e.g., `<account>.dkr.ecr.<region>.amazonaws.com/nodejs-web-app`), `ACTIVE_COLOR` (`blue` or `green`), optional `COLOR_TAG_BLUE`, `COLOR_TAG_GREEN` (override tags per color; empty uses `IMAGE_TAG`)
...
5) `helm upgrade --install` with `image.repository`, `image.tag`, per-color tags, and `activeColor`; creates namespace if missing.
6) Install ArgoCD CLI; login via token or username/password.
7) `argocd app create --upsert` using chart path, image overrides, per-color tags, and `activeColor`; then `argocd app sync` and `argocd app wait` for health.
...
### Blue/Green usage cheat-sheet
- Deploy both colors with different images: set `COLOR_TAG_BLUE=<tagA>`, `COLOR_TAG_GREEN=<tagB>`, `ACTIVE_COLOR=blue` (or `green`).
- Switch traffic: update `ACTIVE_COLOR` to the other color and rerun the workflow (or change in ArgoCD app values).
- If you only want one color live, set the other color’s `enabled: false` in [values.yaml](chart/values.yaml) (blue/green blocks).
